...
from main.constants.auth import DefaultGroups
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--group',
            required=False,
            choices=DefaultGroups.all_keys(),
            default=DefaultGroups.user.name
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if options['verbosity'] >= 1:
                ...
                # add to group, if no group name provided, use default user group
                group_name = options.get('group')

                group = Group.objects.get(name=group_name)
                group.user_set.add(user)
                self.stdout.write(f"User added to group '{group_name}' successfully.")
